...
export default function ItemModal({ item, onClose }) {
  ...
  <>
    <Dialog open={true} onOpenChange={(open) => {
      // Only allow closing if ManualPriceModal is not open
      if (!open && !showPriceModal) {
        onClose()
      }
    }}>
      <DialogContent 
        className="max-w-4xl bg-slate-900 border-slate-700 max-h-[90vh] p-0 flex flex-col"
        onInteractOutside={(e) => {
          // Prevent closing when clicking outside if price modal is open
          if (showPriceModal) {
            e.preventDefault()
          }
        }}
      >
        {/* HEADER */}
        <DialogHeader className="border-b border-slate-700 pb-3 sm:pb-4 px-4 sm:px-6 pt-4 sm:pt-6 flex-shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-4">
            <div className="flex-1 min-w-0">
              <DialogTitle className="text-white text-lg sm:text-2xl font-bold break-words">
                {item["EN-US"]}
              </DialogTitle>
            </div>
            <Badge className={`${getTierColor(item.UniqueName)} shadow-md text-xs sm:text-sm px-2 sm:px-3 py-1 flex-shrink-0 w-fit`}>
          ...
          <div className="space-y-4 sm:space-y-6 py-3 sm:py-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              <div className="lg:col-span-1">
                <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-4 sm:p-6 border border-slate-700 h-full flex items-center justify-center">
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 sm:p-5 border border-slate-700">
                  <h4 className="text-sm sm:text-base font-bold text-blue-400 mb-3 sm:mb-4 flex items-center gap-2">
                    <span className="w-1 h-4 sm:h-5 bg-blue-400 rounded"></span>
                    Crafting Requirements
                  </h4>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4">
                    ...
                    <div className="bg-slate-900/50 rounded-lg p-2 sm:p-3 text-center col-span-2 sm:col-span-1">
                ...
                <div className="bg-gradient-to-br from-amber-900/20 to-slate-800/50 rounded-xl p-4 sm:p-5 border border-amber-700/30">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4 mb-3 sm:mb-4">
                    <h4 className="text-sm sm:text-base font-bold text-amber-700 flex items-center gap-2">
                      <span className="w-1 h-4 sm:h-5 bg-amber-400 rounded"></span>
                      Crafting Economics
                      {isManualCalculation && (
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      className="border-amber-600 text-amber-700 hover:bg-amber-600/20 hover:text-amber-300 transition-colors w-full sm:w-auto text-xs sm:text-sm"
                    >
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      ...
                      <div className="flex justify-between items-center py-2 border-b border-slate-700/50">
                        <div className="flex flex-col">
                          <span className="text-xs sm:text-sm text-slate-300">Return Value</span>
                      ...
                      <div className="flex justify-between items-center pt-3 mt-2 border-t-2 border-slate-600">
                        <span className="text-sm sm:text-base font-bold text-white">Net Profit</span>
                        <span
                        ...
                        >
                          ...
                          <span className="text-xs sm:text-sm text-slate-400 ml-1">silver</span>
                        </span>
                      </div>
                    </div>
                  ) : (
                    <div className="bg-slate-900/50 rounded-lg p-4 sm:p-6 text-center border border-slate-700/50">
                      <div className="text-slate-500 mb-2">
                        <svg className="w-10 sm:w-12 h-10 sm:h-12 mx-auto mb-3 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <p className="text-sm font-medium text-slate-400">Economic Data Unavailable</p>
                        <p className="text-xs text-slate-500 mt-2">
                          Price data cannot be calculated due to low market activity,<br className="hidden sm:inline" />
                          missing resource prices, or item rarity.
                        </p>
                      </div>
                      <div className="mt-4 grid grid-cols-2 gap-2 sm:gap-3 text-xs">
            ...
            <div className="bg-slate-800/50 rounded-xl p-4 sm:p-5 border border-slate-700">
              <h4 className="text-sm sm:text-base font-bold text-green-400 mb-3 sm:mb-4 flex items-center gap-2">
                <span className="w-1 h-4 sm:h-5 bg-green-400 rounded"></span>
                Required Resources
              </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-3">
                {displayItem.CraftingRequirements.craftresource.map((res, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between bg-slate-900/70 rounded-lg p-3 sm:p-4 border border-slate-700/50 hover:border-slate-600 transition-colors gap-2"
                  >
        ...
        {/* FOOTER */}
        <div className="flex justify-end pt-3 sm:pt-4 border-t border-slate-700 flex-shrink-0 px-4 sm:px-6 pb-4 sm:pb-6">
          <Button
            onClick={onClose}
            variant="outline"
            className="border-slate-600 text-slate-600 hover:bg-slate-700 hover:text-white transition-colors w-full sm:w-auto text-sm"
          >
